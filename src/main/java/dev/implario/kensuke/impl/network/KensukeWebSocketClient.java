...
import dev.implario.kensuke.impl.network.packet.PacketAuth;
import dev.implario.kensuke.impl.network.packet.PacketError;
import dev.implario.kensuke.impl.network.packet.PacketOk;
import dev.implario.kensuke.impl.network.packet.PacketUseScopes;
...
public class KensukeWebSocketClient extends SimpleChannelInboundHandler<WebSocketFrame> {
	...
	private final Deque<KensukePacketFrame> commandQueue = new ConcurrentLinkedDeque<>();

	private final Multimap<Class<? extends KensukePacket>, BiConsumer<Dialogue, KensukePacket>> listenerMap = HashMultimap.create();
	...
	public ChannelFuture connect(KensukeConnectionData connectionData) {
	...
	public void write(KensukePacketFrame frame) {
		if (!isActive()) {
			queuePacket(frame);
			return;
		}
	...
	public <T extends KensukePacket> void addListener(Class<T> clazz, BiConsumer<Dialogue, T> listener) {
		validateListener(clazz, listener);
		listenerMap.put(clazz, (BiConsumer<Dialogue, KensukePacket>) listener);
	}

	public <T extends KensukePacketFrame> void removeListener(Class<T> clazz, BiConsumer<Dialogue, T> listener) {
		validateListener(clazz, listener);
	...
	public void writeBatch(KensukePacketFrame... packages) {
		writeBatch(Arrays.asList(packages));
	}

	public void writeBatch(Collection<KensukePacketFrame> packets) {
	...
	public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
		logger.log(Level.SEVERE, "Kensuke exception", cause);
	}
	...
	private void sendHandshake() {
		...
		KensukePacketFrame frame = toKensukeFrame(greetingPackage);
		CompletableFuture<KensukePacket> authFuture = new CompletableFuture<>();
		...
		authFuture.thenAccept(packet -> {
			if (packet instanceof PacketError) {
				...
				channel.writeAndFlush(toWebSocketFrame(toKensukeFrame(new PacketUseScopes(scopes))));
	...
	private void queuePacket(KensukePacketFrame frame) {
	...
	private void readTextFrame(TextWebSocketFrame webSocketFrame) {
		...
		val frame = GlobalSerializers.fromJson(text, KensukePacketFrame.class);
		val clazz = KensukePacket.getClass(frame.getType());
		if (clazz == null) {
	...
	@SuppressWarnings ("unchecked")
	private void handlePacket(KensukePacketFrame packet) {
	...
	private void notifyListeners(Collection<BiConsumer<Dialogue, KensukePacket>> listeners, KensukePacketFrame frame) {
				...
				.handler(new ChannelInitializer<Channel>() {
					...
					protected void initChannel(Channel ch) {
						...
						ch.pipeline()
								...
								.addLast("handler_boss", KensukeWebSocketClient.this);
					}
				})
	...
	private KensukePacketFrame toKensukeFrame(KensukePacket packet) {
		return new KensukePacketFrame(KensukePacket.getType(packet.getClass()), packet, UUID.randomUUID());
	}

	private WebSocketFrame toWebSocketFrame(KensukePacketFrame kensukeFrame) {
		val json = GlobalSerializers.toJson(kensukeFrame);
		if (debugWrites) {
	...
	private void dispatchPacket(KensukePacketFrame packet) {
	...
	private class 	PacketWriteTask implements Runnable {
		...
		final KensukePacketFrame packet;
